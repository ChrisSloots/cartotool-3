...
//	} );
	...
	function selectInit() {
		$('.duallist').not('.multi-select').each(function() {
			var select = $(this);
...
$('.layer-menu').on('click','.jstree-anchor, .btn',function(){
	var url;
	if($(this).is('.btn')) {
		url = $(this).data('url');
	} else {
		url = $(this).parent('li').data('url');
	}
	$('.container').load(url, function(){
		...
		setTimeout(function(){
			$('[data-depends]:checked').each(function(){
				$($(this).data('depends')).addClass('readonly')
			})
		},0)
	})
})
	...
	/*---------------------------- THUMBS LIBRARY ------------------------------------*/
	/*-------------------------------------------------------------------------------*/
	$('body').append('<div class="media-thumbs-overlay"><div class="media-thumbs-container"><div class="media-library-scroller"><div class="media-library-content"></div></div></div></div>')
	var el;
	$('#content').on('click', '.open-thumbs-library', function(){
		el = $(this);
		console.log(el)

		$('.media-thumbs-container .media-library-content').load('media-thumbs.html', function(){
			loaded();
			// myScroll.refresh();
		});
		if($(this).is('.popup')) {
			$('.media-thumbs-overlay').addClass('popup');
		} else {
			$('.media-thumbs-overlay').removeClass('popup');
		}
		setTimeout(function(){
			$('.media-thumbs-overlay').addClass('active');
		},100)
		return el;
	})
	...
	$('body').on('click','.media-thumbs-overlay, .media-library-overlay', function(){
	...
	/*---------------------------- MEDIA LIBRARY ------------------------------------*/
	/*-------------------------------------------------------------------------------*/
	$('body').append('<div class="media-library-overlay"><div class="media-library-container"><div class="media-library-scroller"><div class="media-library-content"></div></div></div></div>')
	var el;
	$('#content').on('click', '.open-media-library', function(){
		el = $(this);
		console.log(el)

		$('.media-library-container .media-library-content').load('media-library.html', function(){
			loaded();
			// myScroll.refresh();
		});
		if($(this).is('.popup')) {
	...
	function disableDependedEl(el) {
		...
		el.closest('.input-fields').find('[data-depends]').each(function(){
			$($(this).data('depends')).removeClass('readonly')
		})

		if(el.length && el.is(':checked')) {
			$(el.data('depends')).addClass('readonly')
			console.log('checked - ' + $(el.data('depends')))
		} else {
			$(el.data('depends')).removeClass('readonly')
...
var myScroll, myScroll2, iscrollActive = false;
	...
	myScroll2 = new IScroll('.media-thumbs-container', {
		scrollbars: true,
		mouseWheel: true,
		interactiveScrollbars: true,
		shrinkScrollbars: 'scale',
...
//		$('body').append('<div class="popup-overlay"><div class="popup-container"><span class="close">X</span><h3>Wilt u deze rij verwijderen?</h3><div class="clear"></div><a class="btn btn-red outlined delete-row">Delete</a></div></div>');
//		setTimeout(function(){
...
//	}
	...
	// });
		...
		var json_text = JSON.stringify(serObj, null, 2);

		var serObj;
		serialize();
		function serialize() {
			serObj = '[';
			$('#tree_nestable > ul > li').each(function() {
				serObj += '{"id":'+$(this).data('id');
